  ...
  switch (action.type) {
    case "ADD_NOTIFICATION": {
      const { notifications, unreadCount } =
        notificationService.addNotification(action.payload);
      return { notifications, unreadCount };
    }
    case "MARK_AS_READ": {
      const { notifications, unreadCount } = notificationService.markAsRead(
        action.payload,
      );
      return { notifications, unreadCount };
    }
    ...
    case "REMOVE_NOTIFICATION": {
      const { notifications, unreadCount } =
        notificationService.removeNotification(action.payload);
      return { notifications, unreadCount };
    }
    case "CLEAR_ALL": {
