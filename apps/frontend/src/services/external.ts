...
export const externalService = {
  sikka: {
    verifyInsurance: async (
      data: SikkaVerifyInsuranceRequest,
    ): Promise<SikkaVerifyInsuranceResponse> => {
    ...
    checkEligibility: async (
      data: SikkaCheckEligibilityRequest,
    ): Promise<SikkaCheckEligibilityResponse> => {
      const { controller, timeout } = createTimeoutController(TIMEOUT_MS);
      try {
    ...
    verifyBenefits: async (
      data: SikkaVerifyBenefitsRequest,
    ): Promise<SikkaVerifyBenefitsResponse> => {
      const { controller, timeout } = createTimeoutController(TIMEOUT_MS);
      try {
    ...
    processClaim: async (
      data: SikkaProcessClaimRequest,
    ): Promise<SikkaProcessClaimResponse> => {
      const { controller, timeout } = createTimeoutController(TIMEOUT_MS);
      try {
  ...
  retell: {
    getCallStatus: async (
      callId: string,
    ): Promise<RetellCallStatusResponse> => {
      const { controller, timeout } = createTimeoutController(TIMEOUT_MS);
      try {
    ...
    getTranscription: async (
      callId: string,
    ): Promise<RetellTranscriptionResponse> => {
      const { controller, timeout } = createTimeoutController(TIMEOUT_MS);
      try {
        const response = await fetch(
          `/api/retell/calls/${callId}/transcription`,
          {
            signal: controller.signal,
          },
        );
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a dental professional assistant. Summarize the appointment notes in a clear and concise way.",
            },
            {
              role: "user",
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a dental treatment analysis assistant. Analyze the effectiveness of the treatment based on the provided data and similar cases.",
            },
            {
              role: "user",
