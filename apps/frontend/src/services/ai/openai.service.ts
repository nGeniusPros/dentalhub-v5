...
import { AssistantType, OpenAIServiceConfig, AssistantResponse } from './openai.types';
...
export class OpenAIService implements AIService {
  ...
  private initializeAssistants() {
    ...
    const assistantConfigs: [AssistantType, string, string][] = [
      ['brain-consultant', import.meta.env.VITE_OPENAI_BRAIN_CONSULTANT_ID!, import.meta.env.VITE_OPENAI_BRAIN_CONSULTANT_API_KEY!],
      ['marketing-coaching', import.meta.env.VITE_OPENAI_MARKETINGCOACHING_ID!, import.meta.env.VITE_OPENAI_MARKETING_COACHING_API_KEY!],
      ['data-retrieval', import.meta.env.VITE_OPENAI_DATA_RETRIEVAL_ID!, import.meta.env.VITE_OPENAI_DATA_RETRIEVAL_API_KEY!],
      ['profitability', import.meta.env.VITE_OPENAI_PROFITABILITY_APPOINTMENT_ID!, import.meta.env.VITE_OPENAI_PROFITABILITY_APPOINTMENT_API_KEY!],
      ['recommendation', import.meta.env.VITE_OPENAI_RECOMMENDATION_ID!, import.meta.env.VITE_OPENAI_RECOMMENDATION_API_KEY!],
      ['analysis', import.meta.env.VITE_OPENAI_ANALYSIS_ID!, import.meta.env.VITE_OPENAI_ANALYSIS_API_KEY!],
      ['patient-care', import.meta.env.VITE_OPENAI_Patient_Care_Agent!, import.meta.env.VITE_OPENAI_Patient_Care_API_KEY!],
      ['operations', import.meta.env.VITE_Operations_Agent!, import.meta.env.VITE_OPENAI_Operations_Agent_API_KEY!],
      ['staff-training', import.meta.env.VITE_OPENAI_Staff_Training_Agent!, import.meta.env.VITE_OPENAI_Staff_Training_Agent_API_KEY!],
      ['lab-case-manager', import.meta.env.VITE_OPENAI_Lab_Case_Manager_Agent!, import.meta.env.VITE_OPENAI_Lab_Case_Manager_Agent_API_KEY!],
      ['procedure-code', import.meta.env.VITE_OPENAI_Procedure_Code_Agent!, import.meta.env.VITE_OPENAI_Procedure_Code_Agent_API_KEY!],
      ['supplies-manager', import.meta.env.VITE_OPENAI_Supplies_Manager_Agent!, import.meta.env.VITE_OPENAI_Supplies_Manager_Agent_API_KEY!],
      ['marketing-roi', import.meta.env.VITE_OPENAI_Marketing_ROI_Agent!, import.meta.env.VITE_OPENAI_Marketing_ROI_Agent_API_KEY!],
      ['hygiene-analytics', import.meta.env.VITE_OPENAI_Hygiene_Analytics_Agent!, import.meta.env.VITE_OPENAI_Hygiene_Analytics_Agent_API_KEY!],
      ['patient-demographics', import.meta.env.VITE_OPENAI_Patient_Demographics_Agent!, import.meta.env.VITE_OPENAI_Patient_Demographics_Agent_API_KEY!],
      ['osha-compliance', import.meta.env.VITE_OPENAI_OSHA_Compliance_Agent!, import.meta.env.VITE_OPENAI_OSHA_Compliance_Agent_API_KEY!]
    ];
    ...
    console.log('Finished initializing assistants. Total configured:', this.assistants.size);
  }
  ...
  async generateResponse(
    ...
    if (!brainConsultant) {
      console.error('Brain consultant not configured. Available assistants:', Array.from(this.assistants.keys()));
    ...
    try {
      ...
      await this.waitForRunCompletion(brainConsultant, mainThreadId, brainRun.id);
      
      console.log('Getting brain consultant response...');
      const brainMessages = await this.getThreadMessages(brainConsultant, mainThreadId);
      const brainResponse = brainMessages.data[0];
      
      if (!brainResponse || !brainResponse.content || brainResponse.content.length === 0) {
        ...
        try {
          ...
          if (analysis.needsSikkaData) {
            ...
            if (analysis.requiredAgents && analysis.requiredAgents.length > 0) {
              console.log('Running specialized agents:', analysis.requiredAgents);
              const agentResponses = await Promise.all(
                analysis.requiredAgents.map(agentType => 
                  this.runSpecializedAgent(agentType, analysis.agentQueries[agentType], mainThreadId)
                )
              );
              ...
              await this.waitForRunCompletion(brainConsultant, mainThreadId, finalRun.id);
              const finalMessages = await this.getThreadMessages(brainConsultant, mainThreadId);
              responseContent = finalMessages.data[0].content[0].type === 'text' 
  ...
  private async runSpecializedAgent(
    ...
    const assistantId = import.meta.env[`VITE_OPENAI_${agentType.toUpperCase()}_ID`]!;
