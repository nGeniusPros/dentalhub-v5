...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const { data: performance, error } = await query.order("period_start", {
      ascending: false,
    });
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const { id } = validationResult.data;
    const { metric, value, period_start, period_end, notes } =
      recordValidationResult.data;
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const validationResult = staffIdSchema.safeParse(req.params);
    const summaryValidationResult = performanceSummarySchema.safeParse(
      req.query,
    );
    if (!validationResult.success) {
      return res.status(400).json({ error: "Invalid staff ID" });
    ...
    const startDate =
      period === "year"
        ...
        : new Date(
            new Date().setMonth(
              new Date().getMonth() - (period === "quarter" ? 3 : 1),
            ),
          );
    ...
    const summary = performance?.reduce(
      (acc: Record<string, MetricSummary>, curr) => {
        if (!acc[curr.metric]) {
    ...
    const averages = Object.entries(summary || {}).reduce(
      (acc: Record<string, number>, [metric, data]) => {
