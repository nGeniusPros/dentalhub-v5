...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    analytics.new_patients =
      analytics.total_patients - returningPatientIds.size;
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const analytics = campaigns?.map((campaign) => {
      const campaignCommunications = communications?.filter(
        (c) => c.campaign_id === campaign.id,
      );
      const sent = campaignCommunications?.length || 0;
      const delivered =
        campaignCommunications?.filter((c) => c.status === "delivered")
          ?.length || 0;
      const opened =
        campaignCommunications?.filter((c) => c.status === "opened")?.length ||
        0;
      const clicked =
        campaignCommunications?.filter((c) => c.status === "clicked")?.length ||
        0;
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    performance?.forEach((item) => {
      ...
      analytics[item.staff_id][item.metric] =
        (analytics[item.staff_id][item.metric] || 0) + item.value;
    });
