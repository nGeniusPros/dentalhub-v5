...
router.post(
  ...
  asyncHandler(
    async (req: Request<{}, {}, RetellWebhookEvent>, res: Response) => {
...
router.post(
  ...
  validateWebhookSignature,
  asyncHandler(
    async (req: Request<{}, {}, SikkaWebhookEvent>, res: Response) => {
      const event = req.body;
...
router.post(
  ...
  validateWebhookSignature,
  asyncHandler(
    async (req: Request<{}, {}, OpenAIWebhookEvent>, res: Response) => {
      const event = req.body;
...
async function handleTranscription(
  supabase: SupabaseClient<Database>,
  event: RetellTranscriptionEvent,
) {
...
async function handleEligibilityVerified(
  supabase: SupabaseClient<Database>,
...
async function handlePreAuthStatusUpdate(
  supabase: SupabaseClient<Database>,
  event: SikkaPreAuthStatusEvent,
) {
  await supabase
...
async function handleBenefitsUpdate(
  supabase: SupabaseClient<Database>,
  event: SikkaBenefitsUpdateEvent,
) {
  await supabase.from("insurance_benefits").insert({
    patient_id: event.data.patientId,
    insurance_id: event.data.insuranceId,
    update_date: event.data.updateDate,
...
async function handleModerationFlagged(
  supabase: SupabaseClient<Database>,
  event: OpenAIModeratedEvent,
) {
