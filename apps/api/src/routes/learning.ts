...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    let query = req.supabase.from("courses").select(`
      *,
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const { id } = validationResult.data;
    const { title, description, order_index, duration } =
      moduleValidationResult.data;
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const completedModules =
      moduleProgress?.filter((p) => p.completed)?.length || 0;
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    let query = req.supabase.from("learning_resources").select("*");
...
router.put(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const validationResult = certificationIdSchema.safeParse(req.params);
    const updateValidationResult = updateCertificationSchema.safeParse(
      req.body,
    );
    if (!validationResult.success) {
      return res.status(400).json({ error: "Invalid certification ID" });
