...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    let query = req.supabase.from("insurance_providers").select("*");
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!validationResult.success) {
      return res
        .status(400)
        .json({ error: "Invalid insurance verification data" });
    }
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      await req.supabase.from("claim_history").insert({
    ...
    try {
      ...
      return res
        ...
        .json({
          error:
            error?.message ||
            "An unexpected error occurred while updating claim status",
        });
    }
  }),
);
...
router.get(
  "/plan-coverage/:insurance_company_id",
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    try {
