 ...
 */
export async function initiateCall(request: VoiceCallRequest): Promise<CallStatus> {
  try {
    const response = await retellApi.post<RetellApiResponse<CallStatus>>('/calls', {
...
export async function getCallStatus(callId: string): Promise<CallStatus> {
  ...
  return apiCache.get(cacheKey, async () => {
    try {
      const response = await retellApi.get<RetellApiResponse<CallStatus>>(`/calls/${callId}`);
      return response.data.data!;
    } catch (error) {
						throw handleRetellError(error);
				}
 ...
 */
export async function getTranscription(callId: string): Promise<TranscriptionResult> {
  const cacheKey = `getTranscription-${callId}`;
  return apiCache.get(cacheKey, async () => {
    try {
      const response = await retellApi.get<RetellApiResponse<TranscriptionResult>>(
...
async function handleTranscriptionCompleted(result: TranscriptionResult): Promise<void> {
  // Implementation would store transcription result
  console.log('Transcription completed:', result);
}

async function handleAnalysisCompleted(result: AIAnalysisResult): Promise<void> {
