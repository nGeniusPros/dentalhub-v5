...
import { Patient, SikkaApiResponse } from '../integrations/sikka/types.js';
...
interface SikkaPatientResponse {
  items: Patient[];
  total: number;
  page: number;
  limit: number;
...
async function migratePatients() {
  try {
    ...
    while (hasMore) {
      try {
        const response = await axios.get<SikkaApiResponse<SikkaPatientResponse>>(
          `${sikkaConfig.baseUrl}/patients?practice_id=${process.env.SIKKA_PRACTICE_ID}&offset=${offset}&limit=${limit}`,
          {
            headers: {
