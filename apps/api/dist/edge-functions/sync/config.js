function validateConfig(config) {
    const required = ['calendarProvider', 'contactsProvider'];
    const missing = required.filter(key => !config[key]);
    if (missing.length > 0) {
        throw new Error(`Missing required sync configuration: ${missing.join(', ')}. ` +
...
function loadConfig() {
    const config = {
        ...
        contactsProvider: process.env.CONTACTS_PROVIDER,
...
export const { calendarProvider: CALENDAR_PROVIDER, contactsProvider: CONTACTS_PROVIDER, calendarApiKey: CALENDAR_API_KEY, contactsApiKey: CONTACTS_API_KEY, calendarId: CALENDAR_ID, contactsListId: CONTACTS_LIST_ID, } = syncConfig;
// Retry configuration
