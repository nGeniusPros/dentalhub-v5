...
async function storeMedia(buffer, options) {
    try {
...
export async function processMedia(data, options) {
    ...
    return edgeCache.get(cacheKey, async () => {
        try {
            ...
            if (data.type === 'image') {
                ...
                throw handleMediaError(new Error(`Unsupported media type: ${data.type}`), 'UNSUPPORTED_MEDIA_TYPE');
            }
            let url = '';
            if (options.storage) {
