 ...
 */
export async function getRequestKey(appId, appKey, masterId, practiceKey) {
    try {
 ...
 */
export async function getPaginatedData(requestKey, practiceId, endpoint, fields, limit = 500) {
    const allData = [];
    let offset = 0;
    let hasMore = true;
    while (hasMore) {
 ...
 */
export async function getSingleRecord(requestKey, practiceId, endpoint, fields) {
    try {
        await rateLimiter.checkLimit();
        const url = `https://api.sikkasoft.com/v4/${endpoint}`;
        const response = await axios.get(url, {
