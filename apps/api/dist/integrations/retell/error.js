...
export function handleRetellError(error) {
    if (error instanceof AxiosError) {
        const retellError = error.response?.data?.error || {
            code: 'UNKNOWN_ERROR',
            message: 'An unknown error occurred with the Retell API',
        };
        throw new RetellIntegrationError(retellError.message, retellError.code, retellError.details);
    }
    if (error instanceof Error) {
        throw new RetellIntegrationError(error.message, 'INTERNAL_ERROR', { originalError: error });
    }
    throw new RetellIntegrationError('An unexpected error occurred', 'UNEXPECTED_ERROR', { originalError: error });
}
...
export function validateWebhookSignature(signature, body, secret) {
    try {
        ...
        throw new RetellIntegrationError('Failed to validate webhook signature', 'INVALID_SIGNATURE', { error });
    }
}
