...
router.put('/:id/:record_id', asyncHandler(async (req, res) => {
    const validationResult = staffIdSchema.safeParse(req.params);
...
router.delete('/:id/:record_id', asyncHandler(async (req, res) => {
    const validationResult = staffIdSchema.safeParse(req.params);
    const recordValidationResult = recordIdSchema.safeParse(req.params);
    if (!validationResult.success) {
        return res.status(400).json({ error: 'Invalid staff ID' });
...
router.get('/:id/summary', asyncHandler(async (req, res) => {
    ...
    const summary = performance?.reduce((acc, curr) => {
        if (!acc[curr.metric]) {
    ...
    const averages = Object.entries(summary || {}).reduce((acc, [metric, data]) => {
