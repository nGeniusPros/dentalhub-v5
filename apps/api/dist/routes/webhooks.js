...
const router = Router();
router.post('/retell', validateWebhookSignature, asyncHandler(async (req, res) => {
...
async function handleCallStarted(supabase, event) {
    await supabase
        .from('call_logs')
        .insert({
...
async function handleCallEnded(supabase, event) {
    await supabase
        .from('call_logs')
        .update({
        status: 'ended',
...
async function handleTranscription(supabase, event) {
    await supabase
...
async function handleRecording(supabase, event) {
    await supabase
