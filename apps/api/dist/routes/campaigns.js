...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const { data: campaigns, error } = await campaignService.getAllCampaigns(type, status);
    if (error) {
...
router.get('/:id', asyncHandler(async (req, res) => {
    ...
    const campaignService = new CampaignService(req.supabase);
    const { data: campaign, error } = await campaignService.getCampaignById(id);
    if (error) {
        return res.status(500).json({ error: error.message });
    }
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    const { name, type, schedule, audience, content, settings } = validationResult.data;
    const campaignService = new CampaignService(req.supabase);
...
router.put('/:id', asyncHandler(async (req, res) => {
    ...
    const { id } = req.params;
    const { name, type, status, schedule, audience, content, settings, metrics } = validationResult.data;
    const campaignService = new CampaignService(req.supabase);
    const { data: campaign, error } = await campaignService.updateCampaign(id, {
        name,
        type,
        status,
        schedule,
...
router.get('/:id/analytics', asyncHandler(async (req, res) => {
    const validationResult = campaignIdSchema.safeParse(req.params);
    const analyticsValidationResult = campaignAnalyticsSchema.safeParse(req.query);
    if (!validationResult.success) {
        return res.status(400).json({ error: 'Invalid campaign ID' });
    }
    ...
    const campaignService = new CampaignService(req.supabase);
    const { data: analytics, error } = await campaignService.getCampaignAnalytics(id, start_date, end_date);
    if (error) {
        return res.status(500).json({ error: error.message });
    }
...
router.post('/:id/schedule', asyncHandler(async (req, res) => {
    const validationResult = campaignIdSchema.safeParse(req.params);
    const scheduleValidationResult = scheduleCampaignSchema.safeParse(req.body);
    if (!validationResult.success) {
        return res.status(400).json({ error: 'Invalid campaign ID' });
    }
    ...
    const campaignService = new CampaignService(req.supabase);
    const { data: campaign, error } = await campaignService.scheduleCampaign(id, schedule_time);
    if (error) {
        return res.status(500).json({ error: error.message });
    }
    return res.json(campaign);
...
router.post('/:id/test', asyncHandler(async (req, res) => {
    ...
    const campaignService = new CampaignService(req.supabase);
    const result = await campaignService.sendTestCampaign(id, test_recipients);
...
router.post('/:id/resume', asyncHandler(async (req, res) => {
    ...
    const { id } = validationResult.data;
    const campaignService = new CampaignService(req.supabase);
    const { data: campaign, error } = await campaignService.resumeCampaign(id);
    if (error) {
        return res.status(500).json({ error: error.message });
    }
    return res.json(campaign);
